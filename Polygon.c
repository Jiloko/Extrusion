...
void P_draw(Polygon *P){
	...
	if (P->_is_filled){
		...
		if (!(P->_is_closed)){ //P isn't closed
			if (P->_nb_vertices==1)
				glBegin(GL_POINTS);
			else
...
int P_isConvex(Polygon *P){ //Algorithme Facile
	int i;
...
int P_isOnTheLeftOfAllEdges(Polygon *P, Vector M){
	...
	for (i = 0; result && i < P->_nb_vertices - 1; i++)
		result = ! (V_isOnTheRight(M, P->_vertices[i], P->_vertices[i + 1]));
...
void P_rotate(Polygon *P, Vector normal){ //Fausse
